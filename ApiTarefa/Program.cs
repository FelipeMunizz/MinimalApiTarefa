...
app.MapGet("/tarefas/{id}", async (int id, AppDbContext db) => await db.Tarefas.FindAsync(id)
is Tarefa tarefa? Results.Ok(tarefa): Results.NotFound("Tarefa não Encontrada"));

app.MapGet("/tarefas/concluidas", async (AppDbContext db) => await db.Tarefas.Where(t => t.IsConcluida).ToListAsync());

app.MapPost("/tarefas", async (AppDbContext db, Tarefa tarefas) =>
{
...
//classe que coordena a funcionalidade do Entity Framework InMemory
